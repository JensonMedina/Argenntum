...
class Conexion:
    ...
    def consultar_datos_cuenta_bancaria(self, idTipoCuenta):
        sql = "select tipoCuenta, nroCuenta from clientes, tipoCuentas where tipoCuentas.id = %s;"
        valores = (idTipoCuenta,)
        self.cursor.execute(sql, valores)
        tipoCuenta = self.cursor.fetchone()
        return tipoCuenta
    
    def consultar_datos_tarjeta(self, idTipoTarjeta):
        # sql = "select nroTarjeta, vencimiento, codigo, tipo, fechaInicio from clientes, tarjetas where tarjetas.id = 3"
        # valores = (idTipoTarjeta,)
        self.cursor.execute(f"select nroTarjeta, vencimiento, codigo, tipo, fechaInicio from clientes, tarjetas where tarjetas.id = {idTipoTarjeta};")
...
@app.route("/tipoCuenta/<int:idTipoCuenta>", methods=["GET"])
def mostrar_datos_cuenta(idTipoCuenta):
    tipoCuenta = conexion.consultar_datos_cuenta_bancaria(idTipoCuenta)
    if tipoCuenta:
...
@app.route("/tipoTarjeta/<int:idTipoTarjeta>", methods=["GET"])
def mostrar_datos_tarjeta(idTipoTarjeta):
    tarjeta = conexion.consultar_datos_tarjeta(idTipoTarjeta)
    if tarjeta:
